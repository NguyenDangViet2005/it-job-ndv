...
function BlogListSection() {
  ...
  useEffect(() => {
    async function fetchBlogs() {
      try {
        setLoading(true);
        const response = await blogApi.getAll(currentPage, postsPerPage, selectedCategoryId ?? undefined);
    ...
    <div className="space-y-12">
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-2 relative aspect-[16/10] group overflow-hidden">
            <Link href={`/blog/${featuredBlogs[0].id}`}>
              ...
              <div className="absolute bottom-0 p-6 text-white space-y-2">
                <Badge className="bg-red-600 hover:bg-red-700 rounded-none border-none text-xs uppercase font-bold">Nổi bật</Badge>
                <h2 className="text-xl md:text-2xl font-semibold leading-tight line-clamp-2 uppercase tracking-tighter">
                  {featuredBlogs[0].title}
                </h2>
                <div className="text-xs text-gray-300 font-medium">{formatDate(featuredBlogs[0].createdAt, 'long')}</div>
              </div>
            </Link>
          </div>
        )}
        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
          {featuredBlogs.slice(1, 3).map((blog) => (
            <div key={blog.id} className="relative aspect-square md:aspect-auto h-full group overflow-hidden">
              <Link href={`/blog/${blog.id}`}>
                ...
                <div className="absolute bottom-0 p-4 text-white">
                  ...
                  <div className="text-[10px] text-gray-400 mt-2 font-medium">{formatDate(blog.createdAt, 'long')}</div>
                </div>
              </Link>
            </div>
          ))}
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-4">
          <h2 className="text-2xl font-black uppercase tracking-tighter text-foreground">Mới nhất</h2>
            ...
            <div key={title} className="space-y-6">
              ...
              <div className="relative aspect-video group overflow-hidden">
                <Link href={`/blog/${sectionBlogs[0].id}`}>
                  <Image src={sectionBlogs[0].image || ""} alt="" fill className="object-cover"/>
              ...
              <div className="space-y-4">
                {sectionBlogs.slice(1).map(b => (
                  <Link key={b.id} href={`/blog/${b.id}`} className="flex gap-4 group">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-[9px] text-muted-foreground font-medium">{formatDate(b.createdAt)}</p>
                    </div>
                  </Link>
                ))}
              </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 pt-8">
           ...
           <div className="text-xs font-bold px-4">TRANG {currentPage} / {totalPages}</div>
           <Button 
            variant="outline" 
            className="rounded-none h-8 px-3"
...
const BlogSkeleton = () => (
  <div className="space-y-12">
    ...
    <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
      {[1, 2, 3, 4, 5].map(i => <Skeleton key={i} className="aspect-video rounded-none" />)}
    </div>
  </div>
);
